...
async function handleApi(req, res, url) {
  ...
  if (orderId && req.method === "POST") {
    await readBody(req);
    const result = await withDatabaseAsync(async (db) => {
      const user = requireUser(req, db, ["worker"]);
  ...
  if (url.pathname === "/api/admin/queue/reorder" && req.method === "POST") {
    const body = await readBody(req);
    const result = withDatabase((db) => {
      const user = requireUser(req, db, ["admin"]);
      return reorderQueue(db, user.id, body.orderIds);
    });
    sendJson(res, 200, { queue: result });
    return;
  }
  ...
  let restockSku = orderIdFrom(url.pathname, "/api/admin/restock/", "/ordered");
  if (restockSku && req.method === "POST") {
    const body = await readBody(req);
    const result = withDatabase((db) => {
      const user = requireUser(req, db, ["admin"]);
      return confirmProductOrdered(db, decodeURIComponent(restockSku), user.id, body);
    });
    sendJson(res, 200, { profile: result });
    return;
  }

  restockSku = orderIdFrom(url.pathname, "/api/admin/restock/", "/receive");
  if (restockSku && req.method === "POST") {
    const body = await readBody(req);
    const result = withDatabase((db) => {
      const user = requireUser(req, db, ["admin"]);
      return receiveProductStock(db, decodeURIComponent(restockSku), user.id, body);
    });
    sendJson(res, 200, result);
    return;
  }
